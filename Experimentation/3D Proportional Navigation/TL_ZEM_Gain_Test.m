...
for k = 1:length(M)
    ...
    for j = 1:length(N)
        ...
        for i = 1:nt-1
            ...
            k1 = dt * TrueProNav(t, x, N(j), aT);
            k2 = dt * TrueProNav(t, x + (1/2)*k1, N(j), aT);
            k3 = dt * TrueProNav(t, x + (1/2)*k2, N(j), aT);
            k4 = dt * TrueProNav(t, x + k3, N(j), aT);
...
for l = 1:length(M)
    ...
    title(['Target Leading True ProNav Gain Tuning, ', num2str(M(l)), '*g'])
    subtitle(['Minimum Miss Distance = ', num2str(min_mdist), 'ft, with gain N = ', num2str(min_gain)])
end 
