...
for i = 1:numTimePts
    ...
    pause(0.1); % Adjust to control animation speed
end
...
function x_dot = MissileDynamicModel(x, t)
    ...
    gamma = x(5);
    px_t = x(6);   py_t = x(7);   % Target position
    Vx_t = x(8);   Vy_t = x(9);   % Target velocity
    theta = x(10);
    ...
    LOS_dot_vec = (LOS_dot.*LOS_mag-LOS.*(dot(LOS,LOS_dot)/LOS_mag))./LOS_mag^2;
